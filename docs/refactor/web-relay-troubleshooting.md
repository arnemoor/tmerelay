...
- **Stream Errored / Conflict / status 409–515:** WhatsApp closed the socket because another session is active or creds went stale. Run `warelay logout` then `warelay login --provider wa-web` and restart the relay.
- **Logged out:** Console prints "session logged out"; re-link with `warelay login --provider wa-web`.
...
## Helpful commands
- Start relay WhatsApp Web: `pnpm warelay relay --provider wa-web --verbose`
- Show who is linked: `pnpm warelay relay --provider wa-web --verbose` (first line prints the linked E.164)
- Logout (clear creds): `pnpm warelay logout`
- Relink: `pnpm warelay login --provider wa-web`
...
## If it keeps failing
1) `warelay logout` → `warelay login --provider wa-web` (fresh QR link).
