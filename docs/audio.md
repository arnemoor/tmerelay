...
- **Optional transcription**: If `inbound.transcribeAudio.command` is set in `~/.warelay/warelay.json`, warelay will:
  1) Download inbound audio (WhatsApp Web or WhatsApp Twilio) to a temp path if only a URL is present.
...
- Outbound caps: WhatsApp Web can send audio/voice up to 16 MB (sends as a voice note with `ptt: true`); WhatsApp Twilio still uses the 5 MB media host guard.
...
- WhatsApp Twilio paths are hosted URLs; WhatsApp Web paths are local. The temp download uses HTTPS for WhatsApp Twilio and a temp file for WhatsApp Web-only media.
