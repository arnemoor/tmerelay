...
import type { WebMonitorTuning } from "../provider-web.js";
 ...
 *
 * This enables cross-provider session sharing where both WhatsApp Web and Telegram
 * can message the same Pi RPC session simultaneously.
 */
  ...
  const monitorPromises = providers.map(async (provider) => {
    try {
      if (provider === "telegram") {
        ...
        const { monitorWebProvider } = await import("../provider-web.js");
