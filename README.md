# warelay — Send, receive, and auto-reply on WhatsApp and Telegram.
...
| Provider | CLI Name | Platform | Account Type | Best For |
|----------|----------|----------|--------------|----------|
...
**A) WhatsApp Web (preferred: no Twilio creds, fastest setup)**
1. Link your account: `warelay login` (scan the QR).
2. Send a message: `warelay send --to +12345550000 --message "Hi from warelay"` (add `--provider wa-web` if you want to force the WhatsApp Web session).
3. Stay online & auto-reply: `warelay relay --verbose` (uses WhatsApp Web when you're logged in; if you're not linked, start it with `--provider wa-twilio`). When a WhatsApp Web session drops, the relay exits instead of silently falling back so you notice and re-login.

**B) WhatsApp Twilio (for delivery status + webhooks)**
...
- **Polling fallback:** `relay` polls WhatsApp Twilio when webhooks aren't available; works headless.
- **Status + delivery tracking:** `status` shows recent inbound/outbound; `send` can wait for final WhatsApp Twilio status.

## Command Cheat Sheet
| Command | What it does | Core flags |
| --- | --- | --- |
| `warelay send` | Send a WhatsApp message (WhatsApp Web or WhatsApp Twilio) | `--to <e164>` `--message <text>` `--wait <sec>` `--poll <sec>` `--provider wa-twilio\|wa-web` `--json` `--dry-run` `--verbose` |
| `warelay relay` | Auto-reply loop (poll WhatsApp Twilio or listen on WhatsApp Web) | `--provider <auto\|wa-twilio\|wa-web>` `--interval <sec>` `--lookback <min>` `--verbose` |
| `warelay status` | Show recent sent/received messages | `--limit <n>` `--lookback <min>` `--json` `--verbose` |
| `warelay heartbeat` | Trigger one heartbeat poll (WhatsApp Web) | `--provider <auto\|wa-web>` `--to <e164?>` `--session-id <uuid?>` `--all` `--verbose` |
...
- Example using OpenAI Whisper CLI (requires `OPENAI_API_KEY`):
  ...
  ```
- Works for WhatsApp Web and WhatsApp Twilio providers; verbose mode logs when transcription runs. The command prompt includes the original media path plus a `Transcript:` block so models see both. If transcription fails, the original Body is used.
...
| --- | --- | --- |
| `TWILIO_ACCOUNT_SID` | Yes (WhatsApp Twilio provider) | Twilio Account SID |
...
### Logging (optional)
- File logs are written to `/tmp/warelay/warelay-YYYY-MM-DD.log` by default (rotated daily; files older than 24h are pruned). Levels: `silent | fatal | error | warn | info | debug | trace` (CLI `--verbose` forces `debug`). WhatsApp Web provider inbound/outbound entries include message bodies and auto-reply text for easier auditing.
...
4) Run `pnpm warelay relay --provider auto` (or `--provider wa-web|wa-twilio`) and send a WhatsApp message; warelay will queue the Claude call, stream typing indicators (WhatsApp Twilio provider), parse the result, and send back the text.
...
- If Funnel is not allowed on your tailnet, the CLI exits with guidance; you can still use `relay --provider wa-twilio` to poll without webhooks.
...
- WhatsApp Web provider dropped: rerun `pnpm warelay login`; credentials live in `~/.warelay/credentials/`.
- Tailscale Funnel errors: update tailscale/tailscaled; check admin console that Funnel is enabled for this device.

### Maintainer notes (WhatsApp Web provider internals)
- WhatsApp Web logic lives under `src/web/`: `session.ts` (auth/cache + provider pick), `login.ts` (QR login/logout), `outbound.ts`/`inbound.ts` (send/receive plumbing), `auto-reply.ts` (relay loop + reconnect/backoff), `media.ts` (download/resize helpers), and `reconnect.ts` (shared retry math). `test-helpers.ts` provides fixtures.
- The public surface remains the `src/provider-web.ts` barrel so existing imports keep working.
- Reconnects are capped and logged; no WhatsApp Twilio fallback occurs after a WhatsApp Web disconnect—restart the relay after re-linking.
...
- Personal WhatsApp safety: Automation on personal accounts can be rate-limited or logged out by WhatsApp. Use `--provider wa-web` sparingly, keep messages human-like, and re-run `login` if the session is dropped.
...
- Deploy / keep running: Use `tmux` or `screen` for ad-hoc (`tmux new -s warelay -- pnpm warelay relay --provider wa-twilio`). For long-running hosts, wrap `pnpm warelay relay ...` or `pnpm warelay webhook --ingress tailscale ...` in a systemd service or macOS LaunchAgent; ensure environment variables are loaded in that context.
- Rotating credentials: Update `.env` (Twilio keys), rerun your process; for WhatsApp Web provider, delete `~/.warelay/credentials/` and rerun `pnpm warelay login` to relink.

## Migration from Legacy Provider Names

If you're upgrading from an older version of warelay, update your CLI commands and config files:

| Old Name | New Name | Description |
|----------|----------|-------------|
| `twilio` | `wa-twilio` | WhatsApp via Twilio Business API |
| `web` | `wa-web` | WhatsApp via personal Web session (Baileys) |

**CLI examples:**
```bash
# Old (deprecated, still works)
warelay send --provider twilio --to +1... --message "Hi"
warelay relay --provider web --verbose

# New (recommended)
warelay send --provider wa-twilio --to +1... --message "Hi"
